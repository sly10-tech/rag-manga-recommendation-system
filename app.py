...
with st.sidebar:
    ...
    mode = st.radio("Answer mode", ["extractive", "llm"], index=0,
                    help="Extractive works with no setup. LLM mode uses Gemini API.")
    st.divider()
...
st.title("🔎 RAG-Based Manga & Webtoons Search System")
...
if search_clicked and query.strip():
    ...
    # Filter sources to only show relevant chunks (score >= 0.35)
    threshold = 0.35
    relevant_sources = [(chunk, score) for chunk, score in retrieved if score >= threshold]

    if relevant_sources:
        st.subheader("Sources")
        for chunk, score in relevant_sources:
